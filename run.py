...
Enjoy!
        ...
        if validators.url(url):
            """
            Try part requests the data from stated url and checks if the HTTP
            status is in the success range
            # Uses pandas and to read the content of page and BytesIO to
            convert bytes sent from the url in readable format for pandas
            """
            try:
            ...
            # Displays error when something goes wrong with the HTTP request
            # like a wrong URL or a timeout
            except requests.exceptions.RequestException as e:
                print(f"""
Couldn't download file. Please make sure URL is correct: {e}
                """)
                return None

            # Displays error when file isn't correctly structured or
            # data type is wrong
            except pd.errors.ParserError as e:
                print(f"""
Couldn't read file. Please ensure the file is a CSV file and try again: {e}
                    """)
                return None
...
def check_data(data):
    ...
    try:
        # Checks with two if statements that "Products" column exists
        # and questions exist
        if "Product" not in data.columns:
            raise ValueError("The product column is missing, please add one")
        if len(data.columns) < 2:
            raise ValueError("""
There are no questions in the data, please add at least one
                            """)
...
def calculate_mean_questions(data):
    """
    Function to calculate the mean value for the answers given for each
    question and prints the mean values to the console next to the questions
    """
...
If you already now how it works click on "Start" or
press the "s" button on your keyboard.
Should it be your first time, click on "Show Explanation" or
press the "e" button on your keyboard.
    """)
    # Definition of options for different terminal menus
    main_options = ["[s] Start", "[e] Show Explanation", "[q] Quit"]
...
6. To insert a new file, just go back to the main menu and
